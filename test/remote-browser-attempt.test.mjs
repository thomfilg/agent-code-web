...
test("EC2 executor browser path recovers the worker-owned process after controller replacement", async t => {
  ...
  const processSocket = path.join(root, "process.sock"), controlSocket = path.join(root, "control.sock");
  const heartbeat = path.join(root, "worker-heartbeat"); await writeFile(heartbeat, "");
  const daemon = await new WorkerSupervisorDaemon({ root, processSocket, controlSocket, heartbeat }).listen();
  const control = request => workerSupervisorControl(request, { socketPath: controlSocket });
  const bridgeSource = `import {bridgeWorkerSupervisor} from ${JSON.stringify(new URL("../src/worker-supervisor-bridge.mjs", import.meta.url).href)};if(process.argv[1]?.startsWith("install -d")){for await(const _ of process.stdin){}}else await bridgeWorkerSupervisor({socketPath:${JSON.stringify(processSocket)}});`;
