...
test("Stop agent with a queue interrupts once and immediately sends the next FIFO message without stopping the runtime", async t => {
  const f = await queueFixture(t, { partial: "Work already explained." }), { manager, store, chat, calls } = f;
  const events = []; manager.on("event", event => events.push(event));
  const first = await manager.submit(chat.id, "original request"); await waitFor(() => calls.length === 1);
...
test("Stop agent without queued input preserves messages and runtime and accepts the next message", async t => {
  const f = await queueFixture(t, { partial: "Partial answer survives interruption." }), { manager, store, chat, calls } = f;
  const events = []; manager.on("event", event => events.push(event));
  const first = await manager.submit(chat.id, "remember this request"); await waitFor(() => calls.length === 1);
  await Promise.all([manager.interrupt(chat.id), manager.interrupt(chat.id)]); await first.completion;
  assert.equal(store.get(chat.id).status, "idle"); assert.equal(f.stops(), 0); assert.equal(f.interruptions(), 1);
  assert.equal(store.get(chat.id).messages[0].text, "remember this request");
  assert.equal(store.get(chat.id).messages.at(-1).text, "Partial answer survives interruption.");
  assert.equal(store.get(chat.id).pendingRequest, null);
  await manager.submit(chat.id, "continue"); await waitFor(() => calls.length === 2);
  assert.equal(events.filter(event => event.type === "runtime_started").length, 1);
  f.complete(); await waitFor(() => !manager.isBusy(chat.id));
});
