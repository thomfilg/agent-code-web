...
test("MCP organizations filter grants by primary repository and keep same-provider credentials separate", async t => {
  ...
  const save = organization => mcps.save({ name: "linear", organization, type: "http", url: "https://mcp.linear.app/mcp", authMode: "oauth" });
  ...
  assert.equal(Object.keys(selected).length, 2); assert.ok(selected.relay_linear);
  assert.deepEqual([...mcps.grants.get("future").keys()], [future.id, shared.id]);
  ...
  assert.equal(Object.keys(other).length, 3); assert.deepEqual([...mcps.grants.get("g2i").keys()], [g2i.id, shared.id, stdio.id]);
  ...
  assert.deepEqual([...mcps.grants.get("legacy").keys()], [future.id, shared.id]);
  ...
  const updated = await mcps.save({ ...g2i, organization: "12-apps", name: "moved" }, g2i.id);
