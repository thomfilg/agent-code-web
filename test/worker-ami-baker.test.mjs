...
test("worker recipe removes builder identity, generates new host keys and does not need final IMDS", async () => {
  const recipe = await readFile(new URL("../deploy/aws/worker-cloud-init.yaml", import.meta.url), "utf8");
  for (const required of ["@openai/codex@0.154.0", "@anthropic-ai/claude-code@2.1.222", "cloud-init.disabled", "cloud-init clean --logs --seed --machine-id", "/var/lib/amazon/ssm", "/root/.aws", "-name 'ssh_host_*_key'", "-name 'ssh_host_*_key.pub'", "agent-web-generate-hostkeys", "-t rsa -b 3072", "-t ecdsa -b 256", "-t ed25519", "IMAGE_FINALIZED", "agent-web-heartbeat.service"]) assert.ok(recipe.includes(required), required);
  assert.equal(recipe.includes("ssh-keygen -A"), false);
