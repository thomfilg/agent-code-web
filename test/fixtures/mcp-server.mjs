...
export async function startMcpFixture({ port = 0, requireAuth = true, anonymousInitialize = false, mcpPath = "/mcp", oauthPrefix = "", issuerParameter = true, beforeMcpRequest = null } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      if (url.pathname === `/.well-known/oauth-protected-resource${mcpPath}`) return json(200, { resource: `${origin}${mcpPath}`, authorization_servers: [origin], scopes_supported: ["mcp:read", "mcp:write"] });
      if (url.pathname === "/.well-known/oauth-authorization-server") return json(200, { issuer: origin, authorization_endpoint: `${origin}${oauthPrefix}/authorize`, token_endpoint: `${origin}${oauthPrefix}/token`, registration_endpoint: `${origin}${oauthPrefix}/register`, response_types_supported: ["code"], grant_types_supported: ["authorization_code", "refresh_token"], token_endpoint_auth_methods_supported: ["none", "client_secret_basic"], code_challenge_methods_supported: pkce ? ["S256"] : [], ...(issuerParameter ? { authorization_response_iss_parameter_supported: true } : {}) });
      ...
      if ([`${oauthPrefix}/authorize`, `${oauthPrefix}/approve`].includes(url.pathname)) {
        ...
        if (url.pathname === `${oauthPrefix}/authorize`) {
          res.setHeader("content-type", "text/html"); return res.end(`<!doctype html><title>Fixture OAuth consent</title><h1>Allow Agent Relay to use fixture tools?</h1><a href="${oauthPrefix}/approve?${params.toString()}">Approve access</a>`);
        }
        const code = randomUUID(); codes.set(code, Object.fromEntries(params));
        const redirect = new URL(params.get("redirect_uri")); redirect.searchParams.set("state", params.get("state")); redirect.searchParams.set("code", code); if (issuerParameter) redirect.searchParams.set("iss", origin);
