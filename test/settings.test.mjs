...
test("protected variables never enter worker env; public values and toggles do; concurrent revisions conflict", async t => {
  ...
  const env = await environments.save({ name: "Staging", backend: "local", variables: [{ key: "SERVICE_TOKEN", value: "protected-123", secret: true }, { key: "APP_REGION", value: "test-region", secret: false }, { key: "DISABLED", value: "hidden", secret: false, enabled: false }] });
...
test("GitHub validates access, redacts credentials, preserves primary order and detects revocation", async () => {
  ...
  const status = await gh.connect({ method: "local" }); assert.equal(status.token, undefined);
