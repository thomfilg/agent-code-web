...
test("official SDK client discovers scoped read/write tools and performs both write operations", async t => {
  ...
  const listed = await client.listTools(); assert.deepEqual(listed.tools.map(tool => tool.name).sort(), ["github_create_pull_request", "github_edit_pull_request", "github_get_pull_request_follow_up", "github_publish_branch"]);
