...
test("switching providers retains chat/workspace and hands off history without reusing session IDs", async t => {
  ...
  assert.equal(switched.workspace, chat.workspace); assert.equal(switched.messages.length, 1); assert.equal(switched.title, chat.title);
  ...
  assert.equal(back.model, "gpt-5.6-sol"); assert.equal(back.agentSessionId, null); assert.equal(back.messages.length, 3);
