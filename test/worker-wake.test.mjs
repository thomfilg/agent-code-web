...
test("a worker-only wake sleeps when unused, but visible chat presence defers the timer", async t => {
  ...
  assert.equal(store.get(chat.id).idleKeepAwakeReason, "tab"); assert.equal(store.get(chat.id).idleDeadlineAt, null);
  await new Promise(resolve => setTimeout(resolve, 140)); assert.equal(calls.sleep, 0);
  await manager.setPresence(chat.id, { clientId: "wake_fixture_tab_2", active: false });
  assert.ok(store.get(chat.id).idleDeadlineAt);
