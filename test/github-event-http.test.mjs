...
test("actual HTTP webhook route accepts signed unauthenticated hints, rejects forged bodies, and protects private subscriptions", { timeout: 15000 }, async t => {
  ...
  await app.manager.pullRequests.refresh(f.chat.id, { force: true });
  ...
  const currentRevision = (await app.manager.githubEvents.state(f.chat.id))?.revision ?? 0;
  const patch = (headers, revision = currentRevision) => fetch(endpoint, { method: "PATCH", headers: { "content-type": "application/json", ...headers }, body: JSON.stringify({ repository: "acme/project", number: 7, notifyFailures: true, wakePassing: false, revision }) });
