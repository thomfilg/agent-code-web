...
test("GitHub picker preserves repository order, branches, selection and inline errors", async ({ page }) => {
  ...
  await expect(page.locator("#repository-group-hint")).toContainText("acme → library");
  ...
  await expect(page.locator('[data-section="company:acme"]')).toContainText("Other/library");
  await expect(page.locator('[data-section="company:other"]')).toHaveCount(0);
