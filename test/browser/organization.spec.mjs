...
test("GitHub picker preserves repository order, branches, selection and inline errors", async ({ page }) => {
  ...
  await page.getByRole("checkbox", { name: /^Acme\/web / }).check();
  ...
  await page.getByLabel("Make Acme/web primary").click();
  ...
  await expect(page.locator("#repository-group-hint")).toContainText("Acme → web");
  ...
  await expect(page.locator('[data-section="company:acme"]')).toContainText("web");
  ...
  await expect(page.locator("#selected-repositories .repository-chip").first()).toContainText("web");
