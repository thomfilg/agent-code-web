...
test("legacy GitHub accounts require a single explicit company assignment without another sign-in", async ({ page }) => {
  ...
  await page.route("**/api/github/connections/github_1", async route => {
    const body = route.request().postDataJSON(); writes.push(body); connections[1] = { ...connections[1], name: body.name, companyId: body.companyId, revision: 5 };
  ...
  await page.locator("#github-account-list section").filter({ hasText: "fixture-second" }).getByRole("button", { name: "Assign company", exact: true }).click();
  await expect(page.locator("#github-connection-name")).toHaveValue("second");
  await page.locator("#github-connection-company").selectOption("acme");
  ...
  expect(writes).toEqual([{ revision: 4, name: "Second account renamed", companyId: "acme" }]);
});
