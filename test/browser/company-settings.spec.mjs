...
test("legacy Companies deep link retains durable registration and stable identifiers", async ({ page }) => {
  const errors = []; page.on("pageerror", error => errors.push(error.message));
  await ready(page); await page.locator("#initial-prompt").fill("Keep this draft");
  await page.goto("/?settings-test=legacy#companies");
  await expect(page.locator("#companies-page")).toBeVisible(); await expect(page.locator("dialog[open]")).toHaveCount(0);
  await page.locator("#company-new").click(); await page.locator("#company-form").getByLabel("Company name", { exact: true }).fill("Browser Company");
  await page.locator("#company-form").getByLabel("Company identifier").fill("browser-company");
  ...
  await page.locator("#company-form").getByLabel("Company name", { exact: true }).fill("Browser Company renamed"); await page.getByRole("button", { name: "Save company", exact: true }).click();
  await expect(card).toContainText("Browser Company renamed");
  await page.reload(); await expect(card).toContainText("Browser Company renamed");
...
test("MCP overview filters registered companies and opens only the selected connection's settings", async ({ page, request }) => {
  ...
  try {
    await ready(page); await openSettingsSection(page, "MCP connections"); await switchSettingsCompany(page, "MCP connections", "acme");
    ...
    await page.locator("#mcp-back").click(); await switchSettingsCompany(page, "MCP connections", "other");
    ...
    await page.locator("#mcp-back").click(); await switchSettingsCompany(page, "MCP connections", "g2i");
...
test("legacy MCP needs an explicit company assignment; cards and details fit a small screen", async ({ page }) => {
  ...
  await openSettingsSection(page, "MCP connections"); await switchSettingsCompany(page, "MCP connections", "acme");
  ...
  await page.locator("#mcp-back").click(); await page.getByLabel("Close MCP connections", { exact: true }).click();
  await expect(page.locator("#mcp-dialog")).toBeHidden(); await expect(page.locator("#company-settings-dialog")).toBeVisible();
