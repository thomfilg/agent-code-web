...
const elements = {
  ...
  healthSummary: $("#machine-health-summary"),
  healthDetail: $("#machine-health-detail"),
  input: $("#message-input"),
  send: $("#send-button"),
...
function paintMachineHealth(health) {
  const anomaly = health.anomaly ? health.anomaly.replaceAll("-", " ") : null;
  const agent = health.agent || {}, tool = health.tool, system = health.system || {}, harness = health.harness;
  elements.health.classList.toggle("anomalous", Boolean(anomaly));
  elements.healthSummary.textContent = anomaly
    ? `Machine health · ${anomaly}`
    : `Machine health · ${health.worker.state} · ${agent.state}${agent.pid ? ` ${agent.pid}` : ""}`;
  const rows = [
  ...
  elements.healthDetail.replaceChildren(list);
}

function terminalMachineHealth(health) {
  const worker = health?.worker || {};
  return worker.state === "stopped" || worker.ec2?.state === "stopped";
}
...
function updateMachineHealth(chatId) {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      if (healthChatId === chatId) { paintMachineHealth(result); reconcileTerminalMachineHealth(chatId, result); }
    } catch (error) {
...
function renderActive() {
  const chat = state.active;
  updateMachineHealth(chat?.id || null);
...
function renderQueue() {
  ...
  const edit = async body => {
    ...
    if (body.sendNowId) {
      ...
      state.active = { ...state.active, workingStartedAt: action.message?.createdAt || new Date().toISOString() };
      renderQueue(); renderMessages(); renderWorkingStatus();
      void api(`/api/chats/${chat.id}/queue`, { method: "PATCH", body: JSON.stringify(body) }).catch(error => {
        ...
        if (state.active?.id === chat.id) { state.active = { ...state.active, workingStartedAt: previousWorkingStartedAt }; renderQueue(); renderMessages(); renderWorkingStatus(); }
        toast(error.message);
      });
      return;
    }
...
function connectEvents(chatId) {
  ...
  source.onmessage = ({ data }) => {
    ...
    if (event.type === "side_chat_updated") { sideChat.update(event, chatId); }
      ...
      toast(event.text);
    } else if (event.type === "model_catalog_updated") {
      ModelPicker.clearCatalogs(); activeModelPicker.key = null; workspaceSettings.modelPicker.key = null; renderActive();
    } else if (event.type === "chat_deleted") {
      forgetChat(chatId).catch(error => toast(error.message));
    }
  };
  source.onerror = () => {
    if (state.active?.id === chatId && !["stopped", "error"].includes(state.active.status)) elements.detail.textContent = "Live connection interrupted; reconnecting…";
  };
}
