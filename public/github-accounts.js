...
export class GitHubAccounts {
  ...
  async refresh({ updateWorkspace = false } = {}) {
    ...
    try {
      ...
      if (updateWorkspace || connectedRevision(previous) !== connectedRevision(this.connections)) {
        ...
        if (loaded && request === this.refreshRequest && $("#new-chat-dialog").open) await this.settings.loadRepositories();
      }
  ...
  render() {
    ...
    for (const connection of this.connections) {
      ...
      if (connection.connected) card.append(element("p", "Repository access follows this GitHub account’s permissions.", "muted"));
      ...
      if (connection.signIn) actions.append(button("Cancel sign-in", () => this.action(connection.id, () => this.api("/api/github/device/cancel", { method: "POST", body: JSON.stringify({ id: connection.signIn.id }) }))));
        ...
        if (connection.connected) actions.append(button("Rename", () => this.edit(connection)));
  ...
  edit(connection) {
    ...
    $("#github-rename-title").textContent = `Rename ${connection.login || connection.name}`;
  ...
  async save() {
    ...
    try {
      await this.api(`/api/github/connections/${this.editing.id}`, { method: "PATCH", body: JSON.stringify({ revision: this.editing.revision, name: $("#github-connection-name").value }) });
      this.editing = null; $("#github-rename-form").hidden = true;
