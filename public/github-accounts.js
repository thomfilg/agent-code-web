...
export class GitHubAccounts {
  ...
  async start(connection) {
    ...
    await this.action(connection?.id || "new", async () => {
      const result = await this.api("/api/github/device", { method: "POST", body: JSON.stringify(connection ? { id: connection.id, revision: connection.revision, companyId: connection.companyId } : { companyId: this.companyId }) });
