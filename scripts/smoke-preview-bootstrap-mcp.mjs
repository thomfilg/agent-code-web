...
let server, transport, client, phase = "fixture-create";
...
try {
  execFileSync("openssl", ["req", "-x509", "-newkey", "rsa:2048", "-nodes", "-days", "1", "-subj", "/CN=Relay preview fixture",
    "-addext", `subjectAltName=DNS:${relay},DNS:${preview}`, "-keyout", path.join(directory, "key.pem"), "-out", path.join(directory, "cert.pem")], { stdio: "ignore" });
  ...
  transport = new StdioClientTransport({ command: process.execPath, args: [path.join(root, "node_modules/@playwright/mcp/cli.js"), "--config", configPath], cwd: root, stderr: "pipe" });
  transport.stderr?.on("data", () => {}); client = new Client({ name: "preview-bootstrap-fixture", version: "1.0.0" }); await client.connect(transport);
  ...
  await run(`await page.goto(${JSON.stringify(repeated.url)});await page.getByRole('heading',{name:'Authenticated preview app',exact:true}).waitFor({timeout:15000});`);
  ...
  console.log(JSON.stringify({ transport: "official Playwright MCP", syntheticAccountsOnly: true, httpsDistinctSites: true,
  ...
  console.error(JSON.stringify({ ok: false, phase })); process.exitCode = 1;
} finally {
  await client?.callTool({ name: "browser_close", arguments: {} }).catch(() => {});
  await client?.close().catch(() => {}); await transport?.close().catch(() => {});
  bootstrap.close(); for (const socket of sockets) socket.destroy(); if (server) await new Promise(resolve => server.close(resolve));
