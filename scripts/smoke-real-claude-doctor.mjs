...
for (const option of options) assert(["--network-isolated", "--trace", "--alias", "--deny-cleanup", "--deny-permissions", "--skip", "--stop-cleanup", "--application", "--broken-user", "--broken-project"].includes(option));
if (!options.has("--network-isolated")) {
  const result = await exec("/usr/bin/unshare", ["--user", "--map-root-user", "--net", "--pid", "--fork", "--mount-proc", "--kill-child=SIGKILL", "--", process.execPath, process.argv[1], ...options, "--network-isolated"], { timeout: 120000, killSignal: "SIGKILL", maxBuffer: 40000 }).catch(error => {
  ...
  const root = await mkdtemp("/tmp/relay-claude-doctor-"), requests = [], decisions = [], owners = [], controls = [];
  ...
  let manager, store, gatewayServer, failure, shutdown, chat, profile, localFile, auditCommand, expectedTool, appState, appUrl;
  ...
  const respond = async (request, response) => {
    ...
    if (!body.tools?.length && text.includes("Write the title in the predominant language")) { titles++; content = done("Private doctor fixture"); }
      ...
      requests.push(body); assert(requests.length <= 20, "Unexpected doctor inference loop");
      ...
      if (phase === "application") {
        ...
        if (step === 0) {
          ...
          content = tool("AskUserQuestion", question("Cleanup", `Audit: one duplicated local instruction; ${broken ? "one invalid settings JSON (reported only; repair was not requested)" : "settings JSON parses"}. Remove only the duplicate 'Use npm test.' line from CLAUDE.local.md, retaining the safety rule?`, cleanupChoices)); step++;
        } else if (step === 2) {
        ...
        } else if (step === 5.5 && !content) { check(/RELAY_DOCTOR_LOCAL/); content = tool("Write", { file_path: localFile, content: `${JSON.stringify(updatedLocal, null, 2)}\n` }); step = 6; }
  ...
  try {
      ...
      spawn(command, args, options) {
        ...
        if (command === config.claude.bin && args.includes("--print") && options.cwd === current.workspace) {
          ...
          child.stdout.on("data", chunk => { buffer += chunk; const lines = buffer.split("\n"); buffer = lines.pop(); for (const line of lines) try {
            const event = JSON.parse(line); if (event.type === "control_response" && event.response?.response?.effective) controls.push(event.response.response);
          } catch {} });
        }
        return child;
      },
      ...
      while (!finished && !failure && Date.now() < deadline) {
        ...
        if (pending && !seen.has(pending.requestId)) {
          ...
          if (expectedTool.name === "AskUserQuestion") {
            ...
            assert.deepEqual(JSON.parse(await readFile(localFile, "utf8")), localSettings, "No permission write before separate consent");
            if (stopCleanup) {
            ...
            assert(["Bash", "Write"].includes(expectedTool.name));
            if (expectedTool.name === "Write") assert([localFile, `${chat.workspace}/CLAUDE.local.md`].includes(input.file_path));
    ...
    assert.deepEqual(JSON.parse(await readFile(localFile, "utf8")), permissions && !cancelled ? updatedLocal : localSettings);
    if (broken) assert.equal(await readFile(configFile, "utf8"), '{"env": {"RELAY_INVALID": ');
    else assert.deepEqual(JSON.parse(await readFile(`${profile}/settings.json`, "utf8")), userSettings);
