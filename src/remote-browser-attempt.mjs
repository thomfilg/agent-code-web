  ...
  async #open(chat) {
    ...
    attempt.context = processId => {
      ...
      const context = {
        ...
        dispose: async ({ receipt = context.receipt } = {}) => {
          ...
          if (receipt) {
            ...
            if (!released) {
              const result = await this.control({ action: "release", processId, processInstanceId: receipt.processInstanceId, leaseId: currentLease.id });
              if (result?.released !== true) throw failure("process release was not confirmed");
              released = true;
            }
          } else {
            const current = await this.control({ action: "status" });
