...
export class ClaudeAdapter {
  ...
  machineHealth() {
    const child = this.child || this.turnSession?.child || this.applicationSession?.child;
  ...
  async forceInterrupt() {
    ...
    (this.turnSession?.requests || this.applicationSession?.requests)?.cancel();
    this.agents?.close(); this.agents = null;

    // A native control can occasionally stop answering even though the CLI
    // process remains alive. Send now is an explicit priority handoff: end
    // only that CLI process group and resume from its checkpoint on next send.
