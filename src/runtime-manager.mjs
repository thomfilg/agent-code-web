...
import { workerActivity, workerActivityDetail } from "./worker-activity.mjs";
import { localSystemMetrics, machineHealthSnapshot } from "./machine-health.mjs";
...
export class RuntimeManager extends EventEmitter {
  ...
  #activity(chatId, runtime = this.#runtimes.get(chatId), { ignoreSuspension = false } = {}) {
    return workerActivity({
      chat: this.store.get(chatId), runtime,
      controls: this.#modeChanges.has(chatId) || this.#workerWakes.has(chatId) || (!ignoreSuspension && this.#suspensions.has(chatId))
        || this.#sendingNow.has(chatId) || this.#switching.has(chatId) || this.#interruptions.has(chatId) || this.#queued.has(chatId)
        || this.#draining.has(chatId) || this.#queueClaims.has(chatId) || this.#startupControllers.has(chatId)
        || this.#previewStops.has(chatId) || this.#workerResizes.has(chatId) || this.#workerReconciliations.has(chatId),
      importing: this.#importPending(chatId), forking: this.#forking.has(chatId), side: this.sideChats.busy(chatId),
      browser: Boolean(this.browsers?.hasSession?.(chatId) || this.browsers?.entries?.has(chatId)),
      preview: this.previewActivity.has(chatId), workspace: this.workspacePresence.has(chatId),
    });
  }
  ...
  sendQueuedNow(chatId, itemId) {
    ...
    action.promise = this.#sendQueuedNow(chatId, item, action).finally(async () => {
      ...
      // Stop/control may already have started a drain while Send now was still
  ...
  async #interruptAdapter(runtime) {
    const adapter = runtime?.adapter;
    if (!adapter) return { forced: false };
    if (!adapter.interrupt) throw new Error("This agent does not support turn interruption");
    const graceMs = this.config.sendNowInterruptTimeoutMs || 2_000;
    const killMs = this.config.sendNowForceInterruptTimeoutMs || Math.max(10_000, graceMs * 5);
    const graceful = Promise.resolve().then(() => adapter.interrupt());
    let graceTimer;
    try {
      await Promise.race([graceful, new Promise((_, reject) => {
        graceTimer = setTimeout(() => reject(new Error("Agent interruption timed out")), graceMs);
      })]);
      return { forced: false };
    } catch (error) {
      if (!adapter.forceInterrupt) throw error;
      void graceful.catch(() => {});
      await this.#forceInterruptAdapter(runtime, killMs);
      return { forced: true };
    } finally { clearTimeout(graceTimer); }
  }

  async #forceInterruptAdapter(runtime, timeoutMs = this.config.sendNowForceInterruptTimeoutMs || 10_000) {
    if (!runtime?.adapter?.forceInterrupt) throw new Error("This agent did not confirm interruption and cannot terminate its native process group");
    const forced = Promise.resolve().then(() => runtime.adapter.forceInterrupt());
    let timer;
    const outcome = await Promise.race([
      forced.then(() => ({ done: true }), failure => ({ done: true, failure })),
      new Promise(resolve => { timer = setTimeout(() => resolve({ done: false }), timeoutMs); }),
    ]);
    clearTimeout(timer);
    if (!outcome.done) {
      void forced.catch(() => {});
      throw Object.assign(new Error("Agent process-group termination timed out; the worker remains online and the queue was preserved"), { code: "AGENT_TERMINATION_TIMEOUT" });
    }
    if (outcome.failure) throw outcome.failure;
  }

  async #settleInterruptedTurn(chatId, runtime, turn, alreadyForced = false) {
    if (!turn) return alreadyForced;
    const timeoutMs = this.config.sendNowInterruptTimeoutMs || 2_000;
    let timer;
    const settled = await Promise.race([turn.done.then(() => true), new Promise(resolve => {
      timer = setTimeout(() => resolve(false), timeoutMs);
    })]);
    clearTimeout(timer);
    if (settled) return alreadyForced;
    // A turn can still be in asynchronous preparation before it owns a native
    // process. Fence that preparation directly; an admitted runtime must prove
    // its native process group was terminated first.
    if (!alreadyForced && runtime) await this.#forceInterruptAdapter(runtime);
    await this.#reconcileTerminatedWork(chatId);
    if (this.#submissions.get(chatId) === turn) {
      // The native owner is gone. Fence late provider completion and do not let
      // unrelated cleanup in the abandoned turn retain Running/Stopping state.
      if (runtime) {
  ...
  async #stopNativeChild(chatId, runtime, agents, threadId) {
    try { return await agents.interrupt(threadId); }
      ...
      if (!/timed?\s*out|confirmation|acknowledge/i.test(errorMessage(error)) || !runtime.adapter.forceInterrupt) throw error;
      const timeoutMs = this.config.sendNowForceInterruptTimeoutMs || 10_000;
      const forced = Promise.resolve().then(() => runtime.adapter.forceInterrupt());
      let timer;
      const outcome = await Promise.race([
        forced.then(() => ({ done: true }), failure => ({ done: true, failure })),
      ...
      if (!outcome.done) {
        void forced.catch(() => {});
        throw new Error("Native child process-group termination timed out; the EC2 worker remains online");
      }
      if (outcome.failure) throw outcome.failure;
      await this.#reconcileTerminatedWork(chatId);
      return runtime.adapter.agents?.snapshot?.() || { rootThreadId: this.store.get(chatId)?.agentSessionId, awake: false, threads: [] };
    }
  }

  async #stopAdapterBounded(runtime, options) {
    const timeoutMs = this.config.sendNowForceInterruptTimeoutMs || 10_000;
    const graceful = Promise.resolve().then(() => runtime.adapter.stop(options));
    let timer;
    const outcome = await Promise.race([
      graceful.then(() => ({ done: true }), failure => ({ done: true, failure })),
      new Promise(resolve => { timer = setTimeout(() => resolve({ done: false }), timeoutMs); }),
    ]);
    clearTimeout(timer);
    if (outcome.done && !outcome.failure) return;
    if (!runtime.adapter.forceStop) {
      if (outcome.failure) throw outcome.failure;
      void graceful.catch(() => {});
      throw new Error("Agent process-group stop timed out");
    }
    const forced = Promise.resolve().then(() => runtime.adapter.forceStop());
    let forceTimer;
    const forcedOutcome = await Promise.race([
      forced.then(() => ({ done: true }), failure => ({ done: true, failure })),
      new Promise(resolve => { forceTimer = setTimeout(() => resolve({ done: false }), timeoutMs); }),
    ]);
    clearTimeout(forceTimer); void graceful.catch(() => {});
    if (!forcedOutcome.done) { void forced.catch(() => {}); throw new Error("Agent process group did not stop after TERM and KILL"); }
    if (forcedOutcome.failure) throw forcedOutcome.failure;
  }
  ...
  async #sendQueuedNow(chatId, item, action) {
    ...
    const interrupt = async () => {
      const result = await this.#interruptAdapter(runtime);
      forced ||= result.forced;
      if (result.forced) await this.#reconcileTerminatedWork(chatId);
    };
    ...
    if (turn) {
      ...
      try {
        ...
        await interrupt();
        forced = await this.#settleInterruptedTurn(chatId, runtime, turn, forced);
      } catch (error) { turn.cancelled = false; throw error; }
      this.#emit(chatId, { type: "turn_interrupted" });
    } else {
      if (runtime?.adapter.isBackgroundBusy?.() || runtime?.adapter.hasAwaitedBackgroundWork?.() || runtime?.adapter.agents?.busy?.()) {
        await interrupt();
  ...
  #drainQueue(chatId) {
    ...
    const draining = this.#runQueue(chatId).finally(() => {
      this.#draining.delete(chatId);
      void this.refreshActivity(chatId).catch(() => {});
    });
  ...
  constructor({ store, config, broker, gatewayOrigin, workerBackend = null, adapterFactory = null, github = null, githubWorkers = null, packageRegistry = null, environments = null, models = null, harnessUpdates = null, attachments = null, mcps = null, commands = null, resources = null, agentAccounts = null }) {
    ...
    this.store = store;
    this.config = config;
    // Presence is UI routing only. It must never renew or reschedule a worker,
    // browser, process heartbeat, or idle deadline.
    this.presence = new ChatPresence();
    ...
    this.models = models;
    this.harnessUpdates = harnessUpdates;
  ...
  async agentThreadAction(chatId, action, input = {}) {
    ...
    try {
      ...
      const result = action === "refresh" ? await agents.refresh()
        ...
        : action === "stop" ? await this.#stopNativeChild(chatId, runtime, agents, input.threadId)
  ...
  async machineHealth(chatId) {
    ...
    if (!chat) throw Object.assign(new Error("Chat not found"), { statusCode: 404 });
    const runtime = this.#runtimes.get(chatId);
    const cache = runtime?.machineHealthCache;
    if (cache && Date.now() - cache.at < 2_000) return cache.value || cache.promise;
    const operation = Promise.resolve().then(async () => {
      let system;
      try { system = await runtime?.executor?.machineHealth?.() || localSystemMetrics(runtime?.executor?.workspace || chat.workspace); }
      catch (error) { system = { unavailable: true, reason: /network|ECONN|ENET|EHOST|ETIMEDOUT/i.test(errorMessage(error)) ? "network-failure" : "probe-failed" }; }
      const snapshot = machineHealthSnapshot({ chat: this.store.get(chatId), runtime, activity: this.#activity(chatId, runtime), system });
      if (system.reason && !snapshot.anomalies.includes(system.reason)) { snapshot.anomalies.push(system.reason); snapshot.anomaly ||= system.reason; }
      return snapshot;
    });
    if (runtime) runtime.machineHealthCache = { at: Date.now(), promise: operation, value: null };
    const value = await operation;
    if (runtime?.machineHealthCache?.promise === operation) runtime.machineHealthCache.value = value;
    return value;
  }
  ...
  async #scheduleWorkerIdle(chatId) {
    ...
    if (!this.#awakeWorkers.has(chatId) || this.#runtimes.has(chatId)) return;
    const version = this.#lifecycleVersions.get(chatId) || 0;
    const activity = this.#activity(chatId, null);
    ...
    const updated = await this.store.update(chatId, current => {
      ...
      return { ...runtimeWorkflowPatch(current, "idle"), status: "idle", statusDetail: reason ? workerActivityDetail(reason) : "Environment ready · no message sent to the agent", idleDeadlineAt: deadline, idleKeepAwakeReason: reason };
    });
    if (!this.#awakeWorkers.has(chatId) || this.#runtimes.has(chatId) || (this.#lifecycleVersions.get(chatId) || 0) !== version) return;
    this.publishChat(updated);
    if (!reason) {
      const timer = setTimeout(() => {
        ...
        if (!this.#awakeWorkers.has(chatId) || this.#runtimes.has(chatId)) return;
        if (this.#activity(chatId, null).active) { void this.#scheduleWorkerIdle(chatId).catch(() => {}); return; }
  ...
  async browserExecutor(chatId) {
    ...
    const pending = (async () => {
      ...
      const acquiring = await this.store.update(chatId, current => {
        check();
        const workerLifecycle = beginWorkerLifecycle(current.workerLifecycle, workerLifecycleAction, nowIso());
        workerLifecycleGeneration = workerLifecycle.generation;
        return { workerLifecycle, startupProgress: { startedAt: nowIso(), stages: [] } };
      });
      workerLifecycleGeneration = acquiring.workerLifecycle.generation;
      check();
      ...
      await this.store.update(chatId, current => {
        check();
        const receipt = executor?.acquisitionReceipt || {};
        const worker = receipt.worker || workerIdentityFromMetadata(executor?.metadata);
        return { ...(executor?.metadata ? { runtimeMetadata: executor.metadata } : {}),
          workerLifecycle: finishWorkerLifecycle(current.workerLifecycle, {
      ...
      let cleanup = acquisitionMutation ? "unknown" : "not-required", cleanupError = null;
      if (acquisitionStarted && this.#executors.get(chatId) === pending) {
        try {
          if (acquisitionMutation) { await acquisitionMutation.release(); pending.workerReleased = true; cleanup = "stopped"; }
          else if (this.config.workerBackend !== "ec2") { await this.workerBackend.sleep(chat); pending.workerReleased = true; cleanup = "stopped"; }
        }
        catch { pending.cleanupFailed = true; cleanup = "failed"; cleanupError = new Error("Worker startup failed and its machine could not be stopped. Use Stop to retry cleanup before waking it again."); }
      }
      if ((this.#lifecycleVersions.get(chatId) || 0) === version && this.store.get(chatId)) {
        await this.store.update(chatId, current => ({ startupProgress: failRunningStartup(current.startupProgress),
          ...(workerLifecycleGeneration === null ? {} : { workerLifecycle: finishWorkerLifecycle(current.workerLifecycle, {
            generation: workerLifecycleGeneration, action: workerLifecycleAction, status: "failed",
            mutation: acquisitionMutation?.mutation || "none", cleanup,
            ...(acquisitionMutation?.worker ? { worker: acquisitionMutation.worker } : {}),
          }, nowIso()) }) }));
  ...
  async browserIdle(chatId) {
    ...
    if (!this.#runtimes.has(chatId) && !this.#activity(chatId, null).active && this.store.get(chatId)) {
  ...
  async refreshActivity(chatId) {
    const runtime = this.#runtimes.get(chatId);
    if (runtime && !runtime.busy && ["idle", "running"].includes(this.store.get(chatId)?.status)) await this.#scheduleIdleStop(chatId, runtime);
  ...
  async setMode(chatId, mode, { nextTurn = false } = {}) {
    ...
    try {
      ...
      this.publishChat(updated); return updated;
    } finally { this.#modeChanges.delete(chatId); pending.done.resolve(); await this.refreshActivity(chatId); void this.#drainQueue(chatId); }
  }
  ...
  async #submit(chatId, rawText, attachmentIds = [], queueAction = null, approval = null, githubEvent = null, relayGoalWake = false) {
    ...
    try {
      ...
      // Forced native process-group termination can settle the logical turn even when the old
  ...
  interrupt(chatId) {
    ...
    const action = this.#interruptTurn(chatId).finally(() => {
      this.#interruptions.delete(chatId);
      void this.refreshActivity(chatId).catch(() => {});
    });
  ...
  async #interruptTurn(chatId) {
    ...
    const runtime = this.#runtimes.get(chatId);
    try {
      // Preserve the worker, browser and services. A stuck logical interrupt
      // escalates only to the native process group, never to EC2 shutdown.
      let forced = false;
      if (runtime && (turn || runtime.adapter.isBackgroundBusy?.() || runtime.adapter.hasAwaitedBackgroundWork?.() || runtime.adapter.agents?.busy?.())) {
        ({ forced } = await this.#interruptAdapter(runtime));
        if (forced) await this.#reconcileTerminatedWork(chatId);
      }
      if (turn) await this.#settleInterruptedTurn(chatId, runtime, turn, forced);
      await runtime?.eventQueue;
  ...
  async #hibernate(chatId) {
    ...
    if (chat.archived || this.#previewStops.has(chatId) || runtime?.failing || runtime?.trustReviewing
      || this.#activity(chatId, runtime, { ignoreSuspension: true }).active) {
    ...
    try {
      ...
      if ((this.#lifecycleVersions.get(chatId) || 0) !== version || runtime && this.#runtimes.get(chatId) !== runtime
        || this.#activity(chatId, runtime, { ignoreSuspension: true }).active) throw new Error("The environment changed while hibernation was being prepared");
      if (native.retained) {
  ...
  async stop(chatId, reason = "manual", { skipAdapter = false } = {}) {
    ...
    let workerLifecycleGeneration = null;
    ...
    if (runtime) {
      ...
      if (!skipAdapter) {
        await this.#stopAdapterBounded(runtime, { checkpointVersion: stopVersion }).catch((error) => this.#emit(chatId, { type: "runtime_log", text: `Adapter stop warning: ${errorMessage(error)}` }));
    ...
    try {
      ...
      this.broker.revokeChat(chatId);
      await this.store.update(chatId, current => ({ workerLifecycle: finishWorkerLifecycle(current.workerLifecycle, {
        generation: workerLifecycleGeneration, action: "stop", status: "failed", mutation: "none", cleanup: "failed",
      }, nowIso()) })).catch(() => {});
    ...
    await this.store.update(chatId, current => {
      ...
      return { workerLifecycle: finishWorkerLifecycle(current.workerLifecycle, {
        generation: workerLifecycleGeneration, action: "stop", status: "succeeded", mutation: "stopped", cleanup: "stopped",
        ...(worker ? { worker } : {}),
      }, nowIso()) };
    });
  ...
  async resizeWorker(chatId, requestedType) {
    ...
    action.promise = (async () => {
      try {
        ...
        await this.refreshActivity(chatId).catch(() => {});
  ...
  async #removeChat(chatId) {
    ...
    if (chat.agent !== "mock") {
      ...
      await this.store.update(chatId, current => {
        const workerLifecycle = beginWorkerLifecycle(current.workerLifecycle, "destroy", nowIso());
        workerLifecycleGeneration = workerLifecycle.generation;
        return { workerLifecycle };
      });
      try {
        await this.workerBackend.destroy(this.store.get(chatId));
        await this.store.update(chatId, current => ({ workerLifecycle: finishWorkerLifecycle(current.workerLifecycle, {
          generation: workerLifecycleGeneration, action: "destroy", status: "succeeded", mutation: "destroyed", cleanup: "stopped", worker: null,
        }, nowIso()) }));
      } catch (error) {
        await this.store.update(chatId, current => ({ workerLifecycle: finishWorkerLifecycle(current.workerLifecycle, {
  ...
  async #start(chatId) {
    ...
    try {
      ...
      if (executor && chat.environmentId) {
        ...
        if (!recoveringNative && this.harnessUpdates) {
          runtimeHarness = await this.#startupTask(chatId, "harness", version, () => this.harnessUpdates.prepare(executor, environment, async () => { checkCancelled(); }));
          checkCancelled();
          if (runtimeHarness?.status === "updated") this.#emit(chatId, { type: "model_catalog_updated", harness: runtimeHarness });
    ...
    let adapter;
    try {
      const adapterConfig = executor?.harnessBins ? { ...this.config,
        codex: { ...this.config.codex, bin: executor.harnessBins.codex },
        claude: { ...this.config.claude, bin: executor.harnessBins.claude } } : this.config;
      adapter = this.adapterFactory
        ...
        : new Adapter({ chat, store: this.store, config: adapterConfig, broker: this.broker, gatewayOrigin: this.gatewayOrigin, executor, hooks, restoreFork: forkRecord && !forkRecord.initialized ? forkRecord.bundle : null, savedAgentThreads,
  ...
  async #scheduleIdleStop(chatId, runtime) {
    clearTimeout(runtime.idleTimer); runtime.idleTimer = null;
    if (this.#runtimes.get(chatId) !== runtime || runtime.failing || runtime.trustReviewing) return;
    const reason = this.#activity(chatId, runtime).reason;
    const chat = this.store.get(chatId);
    if (reason) {
      if (chat?.idleKeepAwakeReason !== reason || chat?.idleDeadlineAt || chat?.status !== "idle") {
        const updated = await this.store.update(chatId, current => ({ ...runtimeWorkflowPatch(current, "idle"), status: "idle", statusDetail: workerActivityDetail(reason), idleDeadlineAt: null, idleKeepAwakeReason: reason }));
        if (updated) this.publishChat(updated);
      }
      return;
    }
    ...
    runtime.idleTimer = setTimeout(() => {
      if (this.#activity(chatId, runtime).active) { void this.#scheduleIdleStop(chatId, runtime); return; }
      if (this.#runtimes.get(chatId) !== runtime || runtime.trustReviewing) return;
  ...
  async #suspensionUnavailable(chatId, { admission = false } = {}) {
    ...
    const updated = await this.store.update(chatId, current => {
      ...
      if (!admission && this.#activity(chatId).active) return {};
  ...
  async #agentEvent(chatId, event) {
    ...
    if (event.type === "goal") {
      this.publishChat(await this.store.update(chatId, { goal: event.goal }));
      await this.refreshActivity(chatId); return;
    }
  ...
  async #reconcileTerminatedWork(chatId) {
    const settled = await this.store.update(chatId, current => ({
      pendingRequest: null,
      messages: current.messages.map(message => message.kind === "tool" && ["running", "stopping"].includes(message.meta?.state)
        ? { ...message, meta: { ...message.meta, state: "completed", interrupted: true, resultMissing: true } }
        : message),
      ...workflowPatch({ ...current, pendingRequest: null }),
    }));
    if (settled) this.publishChat(settled);
    await this.agentThreads.reconcileStopped?.(chatId).catch(error => this.#emit(chatId, { type: "runtime_log", text: `Agent snapshot reconciliation warning: ${errorMessage(error)}` }));
    return settled;
  }
  ...
  async #failRuntime(chatId, runtime, error) {
    ...
    try {
      ...
      try {
        ...
        await this.#stopAdapterBounded(runtime, { checkpointVersion: failureVersion }).catch(error => cleanupErrors.push(error));
        await this.#reconcileTerminatedWork(chatId).catch(error => cleanupErrors.push(error));
