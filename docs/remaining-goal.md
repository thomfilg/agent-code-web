...
| --- | --- | --- |
| Auto approvals | Actual Codex Auto turn handles the reported local IPC approval without a manual prompt; Edits/Plan retain their intended policies | Auto is now the default across the new-chat UI, API, persistence and first Codex/Claude turn; live activation and the exact provider-classifier case remain pending |
