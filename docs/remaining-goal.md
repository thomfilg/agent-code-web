...
| --- | --- | --- |
| Auto approvals | Actual Codex Auto turn handles the reported local IPC approval without a manual prompt; Edits/Plan retain their intended policies | Source now makes Codex Auto non-interactive (`never`) inside its existing sandbox; Edits/Plan stay on-request. Installed Codex passed the exact workspace-root `rg` shape with no approval or reviewer call; deployment and selected-account observation remain pending |
