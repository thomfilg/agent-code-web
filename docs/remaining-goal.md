...
| Company-scoped credentials | Multi-company selectors for GitHub, environments and MCPs; separate same-provider connections; server-side denial of excluded companies before any credential use; migration and revocation tests | Implemented in source; unit/API and browser tests pass. Includes cross-company secondary-repository denial and revocation during OAuth refresh. Live activation and explicit legacy scope selection pending |
