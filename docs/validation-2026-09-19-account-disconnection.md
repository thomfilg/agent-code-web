...
A separate encrypted `agent-account-disconnection` intent (owner/account ID only) survives failed credential-row erasure or worker stop. Initialization restores admission denial only for an existing matching owner/account. Cleanup removes the intent only after credential erasure and worker revocation succeed. Deletion removes the intent too; startup ignores foreign-owner and orphan markers. If *all* persistent writes fail, no new intent can be durably recorded: the controller still blocks access in memory and reports failure, but this is not a guarantee across a crash with completely unavailable storage.
...
- 70/70 passed, then an additional concurrent delete/disconnect/refresh case was added and the full new disconnection file passed 16/16: 71 distinct tests covered in these final runs.
