...
Latest full integrated Node revalidation (`457ccc7`): **1,506/1,506 passed**,
...
resize acceptance passed 2/2. The full combined browser rerun and production
publication remain open; unintegrated GitHub events, guest MCP and search work
are not included. See the [current receipt](validation-2026-09-19-ui-personal-integration.md).
...
| 37 | Composer attachments use one row of image thumbnails and file cards above the text, matching the supplied screenshots. Every card is clickable: large image preview; scrollable text-file preview with filename, size and line count, for both draft and sent attachments | Integrated for existing chat and #new; all 12 attachment cases passed in the combined browser run and desktop/mobile images were inspected. Pending full combined rerun and deployment; see [integration receipt](validation-2026-09-19-ui-personal-integration.md) |
| 38 | Drag and drop files and images onto the chat to attach them to the current draft without sending automatically; preserve existing text/attachments and apply the same validation and previews | Integrated with shared picker/paste validation and company-scoped local #new drafts; explicit first-send, retry and navigation guards tested. Pending full combined rerun and deployment; see [receipt and limits](attachment-cards-drop.md) |
| 39 | Add a saved-prompts composer dropdown: truncated prompt rows with per-row (…) edit/delete menus, + Prompt and edit popups, drag-to-reorder, and availability for selected projects or all projects. Clicking a prompt inserts it into the composer without sending | Integrated as 91d0ef7 for existing chat and #new with encrypted owner libraries, revision-safe editing and no-send insertion. Integrated Node 5/5 and browser 11/11 passed; desktop/mobile inspected. Pending full combined rerun/deployment; see [integration receipt](validation-2026-09-19-ui-personal-integration.md) |
| 40 | Search across messages the user wrote and the AI's final answers, with conversation/result navigation. Do not store or index reasoning/chain-of-thought for this feature; exclude tool activity and intermediate responses from results | Implemented in isolated candidate: owner-scoped literal search, explicit Codex/Claude final projection and old-message navigation; 163 Node compatibility tests (including 8 new), 6 browser tests and installed-Claude loopback contract passed. Legacy/unclassified assistant answers are deliberately excluded and disclosed. Integration/publication pending; see [message-search.md](message-search.md). Not deployed |
