...
| 35 | Notify the agent when PR checks fail; if the container is stopped when checks pass, wake it and deliver a GitHub event message | PR77 integrated with default-off per-PR consent, scoped durable delivery, FIFO, Stop fences and explicit uncertain-delivery review. Full local backend/browser integration passed; actual webhook/cloud/selected-agent acceptance and publication remain open. See [integration receipt](validation-2026-09-19-events-search-integration.md) |
