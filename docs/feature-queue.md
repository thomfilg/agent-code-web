...
| 20 | `/goal` and every available native/installed slash command work, without unsupported-terminal placeholders | Reactivated: known Relay controls, Codex actions/skills and reported Claude commands retain their intended paths; unknown commands now fail before message, worker or queue creation in new and active chats. `/status` is explicitly verified as web-only. Remaining provider-runtime/deployed native acceptance stays tracked in `command-support.md` |
...
| 27 | Auto mode handles the reported local IPC/tool approval without manual prompts | Integrated follow-up: Auto is now the browser, API, persistence and adapter default. The new-chat composer validates it before persistence and launches the first Codex/Claude native turn in Auto instead of first creating an Edits chat. Active Claude mode changes still require an acknowledged native control and never answer an already-pending approval. Focused backend/adapter/browser checks pass; publication and the exact selected-account provider-classifier case remain open. See [first-turn Auto receipt](validation-2026-09-20-first-turn-auto.md) |
