...
| 37 | Composer attachments use one row of image thumbnails and file cards above the text, matching the supplied screenshots. Every card is clickable: large image preview; scrollable text-file preview with filename, size and line count, for both draft and sent attachments | Implemented in component branch for existing chat and #new; local attachment tests 12/12 and desktop/mobile inspection passed. Pending integration/deployment; see [receipt and limits](attachment-cards-drop.md) |
| 38 | Drag and drop files and images onto the chat to attach them to the current draft without sending automatically; preserve existing text/attachments and apply the same validation and previews | Implemented with shared picker/paste validation and company-scoped local #new drafts; explicit first-send, retry and navigation guards tested. Pending integration/deployment; see [receipt and limits](attachment-cards-drop.md) |
| 39 | Add a saved-prompts composer dropdown: truncated prompt rows with per-row (…) edit/delete menus, + Prompt and edit popups, drag-to-reorder, and availability for selected projects or all projects. Clicking a prompt inserts it into the composer without sending | Implemented in component branch for existing chat and #new with encrypted owner libraries, revision-safe editing and no-send insertion. Focused Node 5/5 and final browser 11/11 passed; desktop/mobile inspected. Pending integration/deployment; see [receipt and scope](saved-prompts.md) |
