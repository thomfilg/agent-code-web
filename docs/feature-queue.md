...
Previous full integrated Node revalidation (`457ccc7`): **1,506/1,506 passed**,
zero failed/skipped/cancelled, with optional native/guest checks enabled.
Saved prompts also passed 11 focused browser cases, and new dedicated panel
resize acceptance passed 2/2. Those results preceded GitHub events and search;
the follow-up below supersedes their open combined-browser rerun gate. Guest
official MCP remains unintegrated. See the [earlier receipt](validation-2026-09-19-ui-personal-integration.md).

Follow-up `10d00b4` integrates GitHub events and message search. Its full Node
revalidation passed **1,547/1,547**, zero failures/skips/cancellations, including
the enabled native/guest optional checks. Focused integrated batches also passed
38/38 and 30/30. The first combined browser run passed 86/88. After correcting
search fixture consistency and live message-focus retention, application
`06b441b` passed the full **89/89 browser** rerun without retries. Its backend
is unchanged from the 1,547-test candidate. Production publication remains
...
Follow-up `f3e97db` integrates PR #82's child-agent list and conversation popup.
Codex retains direct input/steering; Claude displays verified public child
messages and supports exact-child Stop, with an explicit unsupported-messaging
notice. Component acceptance passed 137/137 Node and 6/6 browser cases; the
installed-Claude smoke demonstrated actual child creation, observer correlation,
child-only Stop and same-parent continuation. Genuine Codex delegation, live
...
| 14 | Improve composer / command UI | Integrated candidate: primary browser/app/agent actions remain visible; repository, changes, link, conversation, appearance, worker and destructive actions are grouped in a labelled Chat settings menu. Responsive containment and command-picker checks pass; current deployed acceptance pending |
...
| 20 | `/goal` and every available native/installed slash command work, without unsupported-terminal placeholders | Reactivated: one shared leading-slash grammar now applies before and after chat creation. `/foo bar` must resolve `/foo` from the effective catalog or fail before message, worker or queue creation; it can never become ordinary agent text. Known Relay controls, Codex actions/skills and reported Claude commands retain their intended paths, including a reported native command as the first message. `/status` is explicitly verified as web-only. Remaining provider-runtime/deployed native acceptance stays tracked in `command-support.md` |
| 21 | Current user requirement: one GitHub connection per Relay company; only agent accounts may span companies, and chats cannot mix companies. GitHub authorizes repositories/branches within the selected connection, without a separate repository-owner allowlist or credential fallback | Company-bound implementation and local read/worker-gateway acceptance are present; deployed selected-account acceptance remains open. The older Sep 18 instruction to remove connection-company assignment was superseded by the user's later single-company requirement; see [reconciliation ADR](adr/2026-09-19-company-github-supersession.md). Do not remove current company isolation |
| 22 | Resize sidebar, chat and third-column panels | Dedicated local browser acceptance 2/2 passed: pointer/keyboard resize, clamped conversation/panel geometry, persisted sidebar width, reset, mobile containment and no prompt/worker action. Existing source retained; deployed interaction acceptance pending |
...
| 27 | Auto mode handles the reported local IPC/tool approval without manual prompts | Auto is the browser, API, persistence and adapter default. Codex Auto now uses native `approvalPolicy: never` with the existing workspace-write/no-network sandbox, so reviewer escalation cannot surface a manual card; Edits/Plan remain on-request. Installed Codex 0.155.0 completed the reported workspace-root `rg --files` shape with zero approval/reviewer calls against deterministic loopback inference. Deployment and selected-account observation remain open. See [first-turn Auto receipt](validation-2026-09-20-first-turn-auto.md) |
...
| 35 | Notify the agent when PR checks fail; if the container is stopped when checks pass, wake it and deliver a GitHub event message | Follow-up in progress: PRs returned by an agent tool auto-bind to that exact chat and wake it for failed checks, merge conflicts and external review/inline/conversation comments until checks pass without conflicts. Own-account activity is filtered; external text is never injected into the event. A fixed read-only scoped worker tool exposes current checks, failure annotations and comments without a token or generic API. Manual unrelated PRs retain default-off controls. Scoped durable delivery, FIFO, Stop fences and explicit uncertain-delivery review remain enforced. Publication and selected-agent production acceptance remain open. See the [automatic follow-up receipt](validation-2026-09-19-automatic-pr-follow-up.md) and earlier [integration receipt](validation-2026-09-19-events-search-integration.md) |
| 36 | Subscribe to GitHub PR/check/auto-merge events for prompt UI status updates, with polling as a reconciliation fallback | PR77 integrated: signed bounded event receiver, canonical status revalidation, durable dedupe and polling fallback. No remote webhook was registered; operator configuration and real delivery acceptance remain open. See [component scope](github-agent-events.md) |
| 37 | Composer attachments use one row of image thumbnails and file cards above the text, matching the supplied screenshots. Every card is clickable: large image preview; scrollable text-file preview with filename, size and line count, for both draft and sent attachments | Integrated for existing chat and #new; all 12 attachment cases passed again in the final 89-case browser run and desktop/mobile images were inspected. Pending deployment; see [integration receipt](validation-2026-09-19-events-search-integration.md) |
| 38 | Drag and drop files and images onto the chat to attach them to the current draft without sending automatically; preserve existing text/attachments and apply the same validation and previews | Integrated with shared picker/paste validation and company-scoped local #new drafts; explicit first-send, retry and navigation guards passed in the final 89-case browser run. Pending deployment; see [receipt and limits](attachment-cards-drop.md) |
| 39 | Add a saved-prompts composer dropdown: truncated prompt rows with per-row (…) edit/delete menus, + Prompt and edit popups, drag-to-reorder, and availability for selected projects or all projects. Clicking a prompt inserts it into the composer without sending | Integrated as 91d0ef7 for existing chat and #new with encrypted owner libraries, revision-safe editing and no-send insertion. Full backend integration and all eight prompt cases plus composer controls passed in the final 89-case browser run; desktop/mobile inspected. Pending deployment; see [integration receipt](validation-2026-09-19-events-search-integration.md) |
| 40 | Search across messages the user wrote and the AI's final answers, with conversation/result navigation. Do not store or index reasoning/chain-of-thought for this feature; exclude tool activity and intermediate responses from results | PR78/79 integrated: owner-scoped literal search, explicit Codex/Claude final projection, old-message navigation and live focus retention. Full backend integration and seven search cases in the final 89-case browser run passed; installed-Claude loopback contract passed. Delivered GitHub events are excluded from user search. Legacy/unclassified assistant answers are deliberately excluded and disclosed. Publication/selected-native-account acceptance pending; see [integration receipt](validation-2026-09-19-events-search-integration.md) |
| 41 | Deleting a worker/container must preserve the chat and its messages outside disposable storage; only explicit chat deletion removes the conversation. Reproduce actual container deletion independently of stop/restart, using disposable fixtures | Partial integrated candidate: fatal-exit transcript/offline child snapshots plus encrypted Codex checkpoints. Actual OCI deletion and installed Codex 0.155 same-ID restoration passed. PR80 additionally proves committed checkpoint recovery across actual controller-process SIGKILL, independent new PostgreSQL pool and another container, with tool counter one and no implicit turn. This direct-RPC fixture is not the full RuntimeManager/service restart; mid-turn, independent child/workflow and deployed cloud/selected-account acceptance remain open. See [process-loss receipt](validation-2026-09-19-native-controller-loss.md) |
| 42 | Explore and implement a third-column panel showing the main agent's active secondary agents, with native status and supported conversation details. Selecting a secondary agent opens a popup/composer for prompts addressed to that agent, including while it is working; retain accessible keyboard navigation and keep the main agent/conversation independent. Investigate actual Claude Code/Claude web and Codex capabilities, reusing item 20's Codex descendant-navigation work where applicable. Do not invent child sessions or claim unsupported native messaging/steering | Both-provider panel/popup integrated locally via PR #82. Component acceptance: 137/137 Node, 6/6 browser, real installed-Claude child creation/observer/Stop/parent continuation. Claude direct input is explicitly unavailable, not emulated. Genuine Codex delegation and selected-account/deployed acceptance remain open; see [capabilities and evidence](native-agents-panel.md) |
...
| 45 | Provide a repeatable AWS deployment script for the complete Relay application, following future-pay's deployment guidance; add reusable AWS support to `12-apps/ci` and keep Relay a thin application-specific consumer | Shared engine and Relay operators implemented. Authorized HTTPS/readiness/anonymous denial, backup/restore, failed-rollout recovery, fresh/resumed worker, isolated native Claude, real guest Chrome and public workspace upload passed. A tagged-image worker-owned supervisor, private SSH bridge and durable Shared Chrome controller recovery are now integrated candidates with same-process local evidence. Baking/deploying that image, native-agent/Node preservation, product consent/selected-account runtime, authenticated deployed transports, combined selected-GitHub→EC2 and remote app forwarding remain open; MVP is not complete |
